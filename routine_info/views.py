...
# 파일 다운로드, 삭제
def download(request, routine_infoId, filename):
    file_path = os.path.join(settings.ROUTINE_INFO_MEDIA_ROOT, routine_infoId + "/" + filename)
...
def deleteFile(request, routine_infoId, filename):
    path = routine_infoId + "/" + filename
    file_path = os.path.join(settings.ROUTINE_INFO_MEDIA_ROOT, path)
    os.remove(file_path)

    msg = "<script>"
    msg += f"alert('{filename} 파일을 삭제했습니다.');"
    msg += f"location.href='/routine_info/{routine_infoId}/update/';";
    msg += "</script>"
