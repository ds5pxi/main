...
# 파일 다운로드, 삭제
def download(request, workout_qId, filename):
    file_path = os.path.join(settings.WORKOUT_Q_MEDIA_ROOT, workout_qId + "/" + filename)
...
def deleteFile(request, workout_qId, filename):
    path = workout_qId + "/" + filename
    file_path = os.path.join(settings.WORKOUT_Q_MEDIA_ROOT, path)
    os.remove(file_path)

    msg = "<script>"
    msg += f"alert('{filename} 파일을 삭제했습니다.');"
    msg += f"location.href='/workout_q/{workout_qId}/update/';";
    msg += "</script>"
...
def index(request, page):
    ...
    if post_by == 'present' :
        ...
        workout_q = workout_q.order_by('-좋아요','-id')

    best_border = Workout_q.objects.order_by("-좋아요")
